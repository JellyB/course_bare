 ...
 * 直播生成回放时回调 使用
 * - 直播 roomId 会对应多个 courseId,用户在直播答题过程中能够获取到对应的 courseId,
 * - 生成直播回放时 每个courseId 会对应一个 新测 courseId.
 * 1.持久化直播课的排名统计信息（json）
 * 2.持久化直播课中每道题的统计信息(json)
 * 3.直播生成回放，给对应的用户生成一个回放答题卡信息
...
public class LiveCallBackController {
    ...
    public Object liveCallBack(@PathVariable Long roomId, @RequestBody List<LiveCallbackBo> liveCallbackBoList) throws ExecutionException, InterruptedException {
